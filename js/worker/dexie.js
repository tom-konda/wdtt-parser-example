        ...
        try {
            switch (type) {
                ...
                case 'search': {
                    ...
                    const resultArray = await db.trains.where('direction').equals(Number(direction)).and((x) => {
                        return operationDatePattern.includes(x.operationDate);
                    }).toArray();
                    const qualifiedTrains = [];
                    // let hourTrainsCount: {[key: number] : number} = [];
                    const hourTrainsCount = new Map();
